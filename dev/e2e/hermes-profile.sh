...
for command in curl incus openssl python3 ssh sudo; do
...
setting() {
  ...
  value="$(yard "$name" config show "$key" | sed -n 's/^effective: //p')"
  [ -n "$value" ] || die "could not resolve $key for $name"
  printf '%s\n' "$value"
}
...
cleanup() {
  ...
  if [ -n "$STATE" ] && [[ "$STATE" = /tmp/subyard-hermes-profile.* ]] \
    && [ -f "$STATE/.marker" ] \
...
STATE="$(mktemp -d /tmp/subyard-hermes-profile.XXXXXX)"
...
install -d -m 0700 "$SUBYARD_CONFIG_HOME/yards"
...
write_yard_config() {
  local name="$1" port="$2" host_root="$STATE/host-$1"
...
HOST_LINKS=
HOST_BASE=$host_root
RESTRICTED_DISK_PATHS=$host_root
FORWARD_SSH_AGENT=0
DEV_SUDO=0
NESTED_E2E_VMS=0
EOF
}
write_yard_config "$SOURCE_YARD" "$source_ssh_port"
write_yard_config "$RESTORE_YARD" "$restore_ssh_port"

printf '  [ .. ] creating and provisioning source named yard\n'
yard "$SOURCE_YARD" init --yes
yard "$SOURCE_YARD" provision hermes --yes
...
if [ "$REQUIRE_CODEX" = 1 ]; then
  ...
  incus file push "$AUTH_STAGE" "$source_instance/home/dev/.codex/auth.json" \
    --project "$source_project" --quiet
  incus exec "$source_instance" --project "$source_project" -- \
...
'
  ...
  incus exec "$source_instance" --project "$source_project" \
    ...
    /opt/hermes-agent/venv/bin/hermes -t terminal -z \
...
incus exec "$source_instance" --project "$source_project" -- \
  hermes-provider-ready --inference-ok
...
sudo -n bash -euc \
  'set -a; . "$1"; set +a; restic check >/dev/null; restic ls "$2" | grep -Fq /hermes-backup.zip' \
  bash "$restic_env" "$snapshot"
...
yard "$RESTORE_YARD" init --yes
yard "$RESTORE_YARD" provision hermes --yes
...
incus file push "$restored_zip" "$restore_instance/var/tmp/hermes-backup.zip" \
...
if [ "$REQUIRE_CODEX" = 1 ]; then
  printf '  [ .. ] proving restored Hermes-owned Codex auth with real inference\n'
  incus exec "$restore_instance" --project "$restore_project" \
    --user 1000 --group 1000 --cwd /srv/hermes/workspace \
    --env HOME=/home/dev --env HERMES_HOME=/srv/hermes \
    --env HERMES_DISABLE_LAZY_INSTALLS=1 -- \
